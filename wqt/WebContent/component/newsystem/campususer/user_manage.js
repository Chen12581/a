var userDialog;
var condition = null, currentOrgId = -1;
...
function doViewByOrg() {
	fastDev.getInstance('orgButton').disable();
	fastDev.getInstance('bizOrgButton').disable();
	var usertree = fastDev.getInstance('usertree');
	usertree.setOptions({
		asyncDataSource : 'org_initGradeOrgTree.action?moduleId=M32&modeId=view'
	});
	usertree.reLoad({
		url : 'org_initFirstGradeOrgTree.action?moduleId=M32&modeId=view'
	});
}

function onOrgTreeAfterLoadInit()
{
	var orgButton = fastDev.getInstance('orgButton');
	var bizOrgButton = fastDev.getInstance('bizOrgButton');
	if (orgButton != null)
	{
	    orgButton.enable();
	}
	if (bizOrgButton != null)
    {
		bizOrgButton.enable();
    }
}
...
function doViewByBusiness() {
	...
	var usertree = fastDev.getInstance('usertree');
	usertree.setOptions({
		asyncDataSource : 'org_initBusinessOrgTree.action?moduleId=M32&modeId=view'
	});
	usertree.reLoad({
		url : 'org_initFirstBusinessOrgTree.action?moduleId=M32&modeId=view'
	});
}
