...
function batchDeleteObject() {
	...
	if (!!objects) {
		...
		{
			if(objects[i].roleId=="0")
			{
				fastDev.alert("不能删除系统角色\"任何用户\"","信息提示","warning");
				return;
			}if(objects[i].roleId=="1")
			{
				
				fastDev.alert("不能删除系统角色\"超级管理员\"","信息提示","warning");
				return;
			}
...
function onRowClick(event, rowindex, data) {
	...
	if (target) {
		...
		case 'modify':
			createWindow({
				src : "modifyrole.html?roleid=" + data.roleId+"&appId="+data.appId+'&orgId=' + data.orgId,
				title : "修改角色",
				width : "610",
				height : "218",
				buttons : [{
