...
function uploadfile(){
	...
	if(file!=null)
	{
	    ...
	    var myurl ="http://www.w7tong.com/upload/fileUploadServlet?kind=case";
	    $('#upload').form('submit',{
	        url:myurl,
	        onSubmit:function(){
	            return $("#upload").form();
...
$(function(){	
	$('#id').val(newsid);	
	$('#cases_updateform').form('load', 'sysCases_selectWqtCasesOne.action?wqtCases.id='+newsid);
	$("#cases_updateform").form({ 
		 onLoadSuccess: function () { 
			 var ue =  UE.getEditor('uecontent');
			 ue.ready(function() {//编辑器初始化完成再赋值
				 ue.setContent($('#casecontent').val());
				});
			 ...
			 var curWwwPath = window.document.location.href;
	            ...
	            //获取带"/"的项目名，如：/uimcardprj
	            var projectName = pathName.substring(0, pathName.substr(1).indexOf('/') + 1);
			   $("#viewimg").attr('src',"/wqt"+$('#caseimg').val()); 
...
//	}
	...
	$("#casecontent").val(newscontent);
	
	$('#cases_updateform').form('submit', {   
		onSubmit: function(){  
