...
$(function(){	
	$('#id').val(newsid);	
	$('#new_editform').form('load', 'sysnews_NewsOne.action?wqtnews.id='+newsid);
	$('#new_editform').form('load', 'sysnews_NewsBookmarkOne.action?wqtNewsBookmark.newsid='+newsid);
	$("#new_editform").form({ 
...
function uploadfile(){
	...
	if(file!=null)
	{
	    ...
	    var myurl ="http://www.w7tong.com/upload/fileUploadServlet?kind=newswqt";
	    $('#upload').form('submit',{
	        url:myurl,
	        onSubmit:function(){
	            return $("#upload").form();
