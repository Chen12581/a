...
function submitForm(){
	...
	//商标名称
	if($("#brandphoto").val()!=''||$("#brandphoto1").val()!=''){
	    $('#brandForm').form('submit', {   
		    ...
		    {  
		        ...
		        parent.window.location.href='brandshop_add.html?brandid='+id;
		    }  
		});
	}else{
		alert('请先上传');
...
function uploadfile(){
	$("#addUpload").removeAttr("onclick");
	var timer=window.setTimeout(function(){
		$("#addUpload").attr("onclick","uploadfile()");
	...
	if(file!=null || file1!=null)
	{
	    ...
	    var myurl ="http://www.w7tong.com/wqt/upload/fileUploadServlet?kind=brand";
	    $('#upload').form('submit',{
	        url:myurl,
	        onSubmit:function(){
	            return $("#upload").form();
	        ...
	        success:function(data){
	            if (data !='false') {
	                $.messager.alert('提示消息','商标上传成功！','info',function (){
	                	 $("#brandphoto").val(data);
	                });
	            }else{
	                $.messager.alert('提示消息','商标上传失败！','info',function (){
...
$.ajax({
	url:"sysBrand_selBrandShopPhoto.action?brandid="+id,
	type:"GET",
	dataType:"json",
	data:{
		'brandidid':id
...
function dodelete(id){
	...
	if(m==true){
		$.ajax({
			type:"get",
			url:"sysBrand_delIprBrandShopphoto.action",	
			dataType:"json",
			data:{
				'iprBrandShopphoto.id':id
			},
			success:function(data){
