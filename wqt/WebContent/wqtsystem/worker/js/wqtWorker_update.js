...
$(function(){	
	$('#workerForm').form('load', 'sysWorker_selWqtWorkerOne.action?wqtWorker.id='+id);
	$("#workerForm").form({ 
		 onLoadSuccess: function () { 
			   ...
			   $("#viewimg").attr('src',localhostPaht+"/wqt"+$('#headurl').val()); 
			   $("#viewimg1").attr('src',localhostPaht+"/wqt"+$('#weixin').val()); 
...
function uploadfile(){
	...
	if(file!=null || file1!=null)
	{
	    ...
	    var myurl ="http://www.w7tong.com/wqt/upload/fileUploadServlet?kind=worker";
	    $('#upload').form('submit',{
	        url:myurl,
	        onSubmit:function(){
	            return $("#upload").form();
	        ...
	        success:function(data){
	            if (data !='false') {
	                $.messager.alert('提示消息','头像上传成功！','info',function (){
	                	 $("#headurl").val(data);
	                	 console.log($("#headurl").val());
	                });
	            }else{
	                $.messager.alert('提示消息','头像上传失败！','info',function (){
	    ...
	    $('#upload1').form('submit',{
	        ...
	        success:function(data){
	            if (data !='false') {
	                $.messager.alert('提示消息','二维码上传成功！','info',function (){
	                	 $("#weixin").val(data);
	                });
	            }else{
	                $.messager.alert('提示消息','二维码上传失败！','info',function (){
	                    window.parent.closefileupload("");
	                });
	            }
	        }
