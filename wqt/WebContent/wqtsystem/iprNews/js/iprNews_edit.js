...
$(function(){	
	$('#id').val(newsid);	
	$('#new_editform').form('load', 'sysIprNews_NewsOne.action?iprnews.id='+newsid);
	$('#new_editform').form('load', 'sysIprNews_NewsBookmarkOne.action?iprNewsBookmark.newsid='+newsid);
	$("#new_editform").form({ 
...
function uploadfile(){
	...
	if(file!=null)
	{
	    ...
	    var myurl ="http://www.w7tong.com/upload/fileUploadServlet?kind=newsipr";
	    $('#upload').form('submit',{
	        url:myurl,
	        onSubmit:function(){
	            return $("#upload").form();
