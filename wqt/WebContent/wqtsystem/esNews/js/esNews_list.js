...
function doSubmitForm()
{
	...
	var url='sysEsNews_selectEsNewsList.action';
	
	url+='?esnews.title='+title
	     +'&esnews.state='+state
	     +'&esnews.kindid='+kindid;
...
function delNews(newsid)
{
	...
	if(m==true){
		$.ajax({
			type:"post",
			url:"sysEsNews_deleteEsNews.action",	
			dataType:"json",
			data:{
				'esnews.id':newsid
			},
...
function  addNews()
{
	window.location.href='esNews_add.html';
}
...
{
	window.location.href='esNews_edit.html?newsid='+newsid;
}
...
function developNews(newsid)
{
	...
	if(m==true){
		$.ajax({
			type:"post",
			url:"sysEsNews_developEsNews.action",	
			dataType:"json",
			data:{
				'esnews.id':newsid,
				'esnews.state':'1',
...
function previewNews(newsid,state)
{
	    if(state=='1' || state=='2')
		{
	    	$.ajax({
				type:"post",
				url:"sysEsNews_selEsNewsById.action",	
				dataType:"json",
				data:{
					'esnews.id':newsid
				},
	   ...
	   if(state=='1' || state=='2')
		{
	    	$.ajax({
				type:"post",
				url:"sysEsNews_updateEsNews.action",	
				dataType:"json",
				data:{
					'esnews.state':2,
					'esnews.id':newsid
